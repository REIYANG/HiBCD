...
HySure_opt.F = F;
HySure_opt.G = G;
HySure_opt.W1 = Lx; HySure_opt.W2 = Ly;
HySure_opt.N = N;
...
MSE_record = zeros(algNum,trial,SNRCase);
...
psnr_curve = zeros(14,128);
SAM_map = zeros(14,1070,1070);
for i = 1:trial
    % --------generate data
    Y = reshape(dataset,[],M)';
    Y_M = F*Y; Y_H = Y*G;
    ...
    for j = 2:1:2
        ...
        [PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j),psnr_curve(caseCode,:),SAM_map(caseCode,:,:)]  = ...
            quality_assessment(reshape(Y_cut',lx,ly,[]),reshape(Y_interpolation',lx,ly,[]),0,1/dsRatio);
        fprintf('Naive:  N=%g --- MSE: %g, time: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j));
        ...
        fprintf('CNMF: N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        ...
        runtime(caseCode,i,j) = etime(clock,timer);
        objRecord{caseCode,i,j} = para_opt.cost_BCD;
        timeRecord{caseCode,i,j} = para_opt.time_record;
        objRecord{caseCode,i,j}(1) = obj_init;
        ...
        fprintf('FUMI: N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        ...
        fprintf('SupResPALM: N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        %
        ...
        fprintf('HiBCD-plain CoSMF(FW+FPG): N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        %
        ...
        fprintf('HiBCD-plain CoSMF(FPG+FPG): N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        ...
        fprintf('HiBCD-plain CoSMF(FW+FW): N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        ...
        [Y_PG,objRecord{caseCode,i,j,:},timeRecord{caseCode,i,j,:},iter_record(caseCode,i,j)] = HiBCD_NNC(HSI_noise,MSI_noise,F,G,N,'A_UPDATE',3,...
            'MAXITER',maxIter,'STOPPING',stopThreshd,'S_INIT',S_init,'A_INIT',A_init,'Gamma',300,'S_UPDATE',2);
        runtime(caseCode,i,j) = etime(clock,timer);
        ...
        fprintf('HiBCD-NNC CoSMF(FW+FPG): N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        ...
        timer = clock;
        [Y_PG,objRecord{caseCode,i,j,:},timeRecord{caseCode,i,j,:},iter_record(caseCode,i,j)] = HiBCD_NNC(HSI_noise,MSI_noise,F,G,N,'A_UPDATE',3,...
            'MAXITER',maxIter,'STOPPING',stopThreshd,'S_INIT',S_init,'A_INIT',A_init,'Gamma',300,'S_UPDATE',3);
        runtime(caseCode,i,j) = etime(clock,timer);
        ...
        fprintf('HiBCD-NNC CoSMF(FPG+FPG): N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
        ...
        fprintf('HiBCD-NNC CoSMF(FW+FW): N=%g --- MSE: %g, time: %g, iteration: %g.\n',N,MSE_record(caseCode,i,j),runtime(caseCode,i,j),iter_record(caseCode,i,j));
        fprintf('               PSNR: %g, ERGAS: %g, SAM: %g.\n',...
            PSNR(caseCode,i,j),ERGAS(caseCode,i,j),SAM(caseCode,i,j));
    end
    ...
    record.SAM = SAM;
    record.ERGAS = ERGAS;
    record.psnr_curve = psnr_curve;
    record.SAM_map = SAM_map;
