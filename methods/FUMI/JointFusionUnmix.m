function [Out,para_opt]= JointFusionUnmix(VXH,VXM,ChInv,CmInv,psfY,psfZ,sizeIM,para_opt_in,G)
...
%     rou=1e5; % The ADMM regularization parameter
    ...
    N_admm=1e3;                % The maximum numbre of ADMM steps
...
%       [E_hyper]= EEA(E_hyper,A_hyper,A_multi,VXH,VXM,psfZ,ChInv,0); % Only HS    
    ...
    obj_n =(norm(VXH-E_hyper*(A_multi*G),'fro')^2+norm(VXM-F*E_hyper*A_multi,'fro')^2)/2; 
    time_record(t+1) = toc+time_record(t);
    tic;
    cost_BCD(t+1) = obj_n;
%     if cost_BCD(t+1)>cost_BCD(t)
%         t
%     end
...
Out.E_multi=E_multi;
Out.A_multi=A_multi;
Out.Y_FUMI = E_hyper*A_multi;
para_opt.cost_BCD=cost_BCD;
para_opt.time_record=time_record;
