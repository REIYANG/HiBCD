function [Y_HiBCD,f_record,time_record,count] = HiBCD_stepSize(HSI,MSI,F,G,N,varargin)
S_init = [];
A_init = [];
stopThreshd = 1e-5;
maxIter = 500;
    ...
    for i=1:2:(length(varargin)-1)
        switch upper(varargin{i})
            ...
            case 'S_UPDATE'
                S_flag = varargin{i+1};
            case 'FPG_STEP_SIZE'
                step_flag = varargin{i+1};
            case 'FW_STEP_SIZE'
                FW_step_flag = varargin{i+1};
            otherwise
...
%         D_S = ind2vec(colIndex,N)-S_ex;
        ...
        elseif FW_step_flag == 2   
            APhi = A*Phi;
            alpha_k =  min(1,-(nabla_S(:)'*D_S(:))/(max(norm(APhi'*thetaG_FFt*APhi,2),delta)*(D_S(:)'*D_S(:))));
        else
            alpha_k =  min(1,-(nabla_S(:)'*D_S(:))/(max(norm(A'*thetaG_FFt*A,2),delta)*(D_S(:)'*D_S(:))));
        end
        S = S_ex+alpha_k*D_S;
    else % (F)PG update
        nabla_S = (AtA*(S_ex*G)-AtY_H)*G'+FA'*(FA*S_ex-Y_M);
        if step_flag==1||step_flag==2
            APhi = A*Phi;
        else
            APhi = A;
        end
        if step_flag==1||step_flag==3
            beta_S = max(norm(APhi'*thetaG_FFt*APhi,2),delta);
        else
            FAPhi = F*APhi;
            beta_S = norm(FAPhi'*FAPhi,2)+theta_G*norm(APhi'*APhi,2);
        end
